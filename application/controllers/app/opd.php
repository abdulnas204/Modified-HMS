...
class Opd extends General{
	...
	public function index($offset = 0){
		...
		{	
				if($patient->nStatus == "Pending"){
					$nStatus = "Consulting";
					$discharge = anchor('app/opd/discharge/'.$patient->IO_ID.'/'.$patient->patient_no,'Confirm Consult',array('class'=>'delete','onclick'=>"return confirm('Are you sure ?')"));
				}else{ 
					$nStatus = "Consulted";
					$discharge = "Consulted";
				}
    ...
    public function printOR2()
    {
        ...
        $this->data['detailsInv'] = $this->billing_model->detailsInv2($InvNo);
        $this->data['getOR'] = $this->billing_model->OR_num($InvNo);
        $this->data['ipdInfo'] = $this->ipd_model->getIPDPatient($iop_no);
        $this->data['particular_cat'] = $this->billing_model->particular_cat();


        $this->load->view("app/opd/printOR2", $this->data);
